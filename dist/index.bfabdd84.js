 ...
 */ var patternizer = {
    ...
    stripe: function(canvas, options) {
        ...
        for(var j = stripes.length - 1; j >= 0; j--){
            var current = stripes[j], opacity = current.opacity / 100 || 0.5, color = patternizer.HEX_RGBA(current.color, opacity), plaid = current.plaid || false, rotate = current.rotation || 0, stripeWidth = current.width, offset = current.offset || 0, gap = current.gap || stripeWidth, tile = stripeWidth + gap, repeats = (diagLength * 2 + offset) / tile;
            // rotate
            ctx.rotate(patternizer.DEGREES_RADIANS(rotate));
            for(var i = 0; i < repeats; i++){
                if (patternizer.isArray(color)) {
                    // I'm hoping its an array of colors
                    var startStripe = tile * i + offset, endStripe = tile * i + offset + stripeWidth, lingrad = ctx.createLinearGradient(startStripe, 0, endStripe, 0), //get the number of color stops needed
                    stopAmount = 1 / (color.length - 1);
                    // put in the color stops evenly across the rectangle
                    for(var k = 0; k < color.length; k++)lingrad.addColorStop(stopAmount * k, color[k]);
                    ctx.fillStyle = lingrad;
                } else // its a plain old string color
                ctx.fillStyle = color;
                // drawing x
...
if (patternizer.supportsCanvas) // prototype the patternizer method onto canvas
...
//# sourceMappingURL=index.bfabdd84.js.map
